...
with st.sidebar:
    ...
    if data_source == '📚 Biblioteca':
        ...
        with st.expander("➕ Nueva familia"):
            ...
            st.markdown("**Archivo obligatorio:**")
            new_crawl = st.file_uploader("📊 Crawl Screaming Frog (CSV)", type=['csv'], key='new_crawl',
                                         help="Crawl con seoFilterWrapper extraído")
            
            st.markdown("**Archivos opcionales:**")
            new_adobe_urls = st.file_uploader("📈 Adobe URLs - Tráfico por URL", type=['csv'], key='new_adobe',
                                              help="Sesiones SEO por URL")
            new_adobe_filters = st.file_uploader("🔍 Adobe Filters - Demanda por filtro", type=['csv'], key='new_filters',
                                                 help="Visitas SEO por filtro/faceta")
            new_gsc = st.file_uploader("🔎 Google Search Console", type=['csv'], key='new_gsc',
                                       help="Datos de rendimiento de GSC")
            new_semrush = st.file_uploader("📉 SEMrush Keywords", type=['csv'], key='new_semrush',
                                           help="Volúmenes de búsqueda")
            
            if st.button("💾 Crear Familia") and new_family_name and new_crawl:
                import tempfile
                
                # Guardar crawl (obligatorio)
                with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as tmp:
                    tmp.write(new_crawl.getvalue())
                    crawl_path = tmp.name
                
                # Guardar archivos opcionales
                ...
                if new_gsc:
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as tmp:
                        tmp.write(new_gsc.getvalue())
                        gsc_path = tmp.name
                
                semrush_path = None
                if new_semrush:
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as tmp:
                        tmp.write(new_semrush.getvalue())
                        semrush_path = tmp.name
                
                with st.spinner("Creando familia..."):
                    try:
                        local_lib = FamilyLibrary()
                        new_meta = local_lib.create_family(
                            ...
                            gsc_file=gsc_path,
                            semrush_file=semrush_path
                        )
                        
                        if library.is_drive_enabled():
                            library.sync_to_drive(new_meta.id)
                        
                        st.success(f"✅ Familia '{new_family_name}' creada")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Error creando familia: {str(e)}")
