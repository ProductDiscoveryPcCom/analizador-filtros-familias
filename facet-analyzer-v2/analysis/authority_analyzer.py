"""
Módulo de análisis de autoridad - v2.3
...
class AuthorityAnalyzer:
    ...
    def _merge_data(self) -> pd.DataFrame:
        ...
        merged = self.crawl.copy()
        
        if len(self.adobe_urls) == 0:
            merged['visits_seo'] = 0
            return merged
        
        # Detectar columna URL en adobe
        url_col_adobe = None
        for col in ['url_full', 'url', 'url_clean']:
            if col in self.adobe_urls.columns:
                url_col_adobe = col
                break
        
        if url_col_adobe is None:
            merged['visits_seo'] = 0
            return merged
        
        # Merge
        merged = merged.merge(
            self.adobe_urls[[url_col_adobe, 'visits_seo']],
            left_on=self.url_col,
            right_on=url_col_adobe,
            how='left'
        )
        merged['visits_seo'] = merged['visits_seo'].fillna(0).astype(int)
        
        # Limpiar columna duplicada
        if url_col_adobe in merged.columns and url_col_adobe != self.url_col:
            merged = merged.drop(columns=[url_col_adobe], errors='ignore')
        
        return merged
    
    def _get_urls_by_status(self, status_code: int) -> pd.DataFrame:
        """Obtiene URLs filtradas por código de estado"""
        if self.status_col in self.merged.columns:
            return self.merged[self.merged[self.status_col] == status_code].copy()
    ...
    def analyze_no_distribution(self, min_traffic: int = 100) -> List[AuthorityLeak]:
        ...
        """
        urls_200 = self._get_urls_by_status(200)
    ...
    def analyze_dead_ends(self) -> Tuple[int, int, Dict[str, int]]:
        ...
        """
        if self.status_col not in self.crawl.columns:
            return 0, 0, {}
        
        urls_404 = self.crawl[self.crawl[self.status_col] == 404]
        
        # Cruzar con Adobe para calcular tráfico perdido
        ...
        if len(self.adobe_urls) > 0:
            ...
            if url_col_adobe and 'visits_seo' in self.adobe_urls.columns:
        ...
        for facet, pattern in patterns.items():
            try:
                count = len(urls_404[
                    urls_404[self.url_col].str.contains(pattern, case=False, na=False, regex=True)
                ])
                if count > 0:
...
def get_wrapper_distribution(crawl_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # reset_index con nombre explícito para compatibilidad con pandas 2.x
