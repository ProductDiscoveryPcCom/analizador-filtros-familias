"""
Módulo de análisis de facetas - v2.3
...
class FacetAnalyzer:
    ...
    def __init__(self,
        ...
        Args:
            ...
            base_url: URL base de la categoría para detección de homepage
        """
        ...
        self.depth_col = 'Nivel de profundidad' if 'Nivel de profundidad' in self.crawl.columns else None
        ...
        # Detectar homepage/página principal
        self.homepage = self._find_homepage()
    
    def _find_homepage(self) -> Optional[pd.Series]:
        """
        Encuentra la página principal de la categoría (L0)
        Usa múltiples estrategias en orden de prioridad
        """
        if len(self.urls_200) == 0:
            return None
        
        # Estrategia 1: Buscar por URL base exacta
        if self.base_url:
            base_matches = self.urls_200[
                self.urls_200[self.url_col].str.rstrip('/') == self.base_url.rstrip('/')
            ]
            if len(base_matches) > 0:
                return base_matches.iloc[0]
        
        # Estrategia 2: Buscar por nivel de profundidad 0
        if self.depth_col and self.depth_col in self.urls_200.columns:
            min_depth = self.urls_200[self.depth_col].min()
            depth_matches = self.urls_200[self.urls_200[self.depth_col] == min_depth]
            if len(depth_matches) > 0:
                # Si hay varias con la misma profundidad, elegir la de URL más corta
                shortest_idx = depth_matches[self.url_col].str.len().idxmin()
                return depth_matches.loc[shortest_idx]
        
        # Estrategia 3: URL más corta (probablemente la categoría principal)
        if len(self.urls_200) > 0:
            # Filtrar URLs que terminan en categoría principal (sin filtros)
            category_urls = self.urls_200[
                ~self.urls_200[self.url_col].str.contains(r'/[^/]+-[^/]+/', na=False, regex=True)
            ]
            if len(category_urls) > 0:
                shortest_idx = category_urls[self.url_col].str.len().idxmin()
                return category_urls.loc[shortest_idx]
        
        # Fallback: primera URL
        return self.urls_200.iloc[0] if len(self.urls_200) > 0 else None
    ...
    def _get_traffic_by_pattern(self, pattern: str) -> int:
        ...
        url_col = None
        for col in ['url', 'url_full', 'url_clean']:
            if col in self.adobe_urls.columns:
                url_col = col
                break
        
        traffic_col = 'visits_seo' if 'visits_seo' in self.adobe_urls.columns else 'visits'
        
        if url_col is None or traffic_col not in self.adobe_urls.columns:
            return 0
    ...
    def _check_in_wrapper(self, pattern: str) -> tuple:
        """
        Verifica si la faceta está en seoFilterWrapper de la página principal
        """
        if self.homepage is None:
            return False, 0
        
        # Buscar columnas de hrefs
        href_cols = [c for c in self.crawl.columns if 'seofilterwrapper_hrefs' in str(c).lower()]
    ...
    def analyze_all_facets(self) -> FacetAnalysisResult:
        ...
        for mapping in self.facet_mappings:
            try:
                ...
                alerts.append(f"⚠️ Error analizando faceta {mapping.facet_name}: {str(e)}")
