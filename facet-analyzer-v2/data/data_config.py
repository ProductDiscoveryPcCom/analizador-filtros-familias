"""
Módulo de configuración de datos - v2.2
...
class FacetMapping:
    ...
    adobe_filter_prefix: Optional[str] = None  # Prefijo para buscar en Adobe Filters
    user_verified: bool = False
    notes: str = ""
    category: str = ""  # Categoría de la faceta (marca, tamaño, precio, etc.)
    is_custom: bool = False  # True si fue creada manualmente por el usuario
...
"""
        for fm in self.facet_mappings[:15]:  # Limitar para no saturar contexto
...
class FacetDetector:
    ...
    # Patrones genéricos organizados por categoría
    # Se expanden dinámicamente basados en los datos
    GENERIC_PATTERNS = {
        # === MARCAS (se detectan dinámicamente) ===
        'brand': {
            'category': 'marca',
            'patterns': [],  # Se llenan dinámicamente
            'suggested_name': 'Marcas',
            'adobe_filter_prefix': 'marcas:',
            'dynamic': True,  # Indica que se detecta de los datos
        },
        
        # === TAMAÑOS / DIMENSIONES ===
        'size_screen': {
            'category': 'tamaño',
            'patterns': [r'pulgadas', r'pequeno', r'grande', r'-pulgadas', r'pantalla-'],
            'suggested_name': 'Tamaño de Pantalla',
            'adobe_filter_prefix': 'pulgadas:',
        },
        'size_capacity': {
            'category': 'tamaño',
            'patterns': [r'litros', r'-l-', r'-litros', r'capacidad'],
        ...
        'memory_ram': {
            'category': 'memoria',
            'patterns': [r'(?:\d+)-?gb-?ram', r'memoria-ram', r'-ram-'],
            'suggested_name': 'Memoria RAM',
            'adobe_filter_prefix': 'memoria ram:',
        },
        'storage': {
            'category': 'almacenamiento',
            'patterns': [r'(?:\d+)-?gb(?!-?ram)', r'(?:\d+)-?tb', r'almacenamiento', r'disco-duro'],
        ...
        'connectivity_bluetooth': {
            'category': 'conectividad',
            'patterns': [r'bluetooth', r'-bt-'],
            'suggested_name': 'Bluetooth',
            'adobe_filter_prefix': 'conectividad:',
        },
        'connectivity_nfc': {
            'category': 'conectividad',
            'patterns': [r'/nfc(?:/|$)', r'-nfc-', r'-nfc$'],
            'suggested_name': 'NFC',
            'adobe_filter_prefix': 'conectividad:',
        },
        'dual_sim': {
        ...
        'condition_new': {
            'category': 'estado',
            'patterns': [r'/nuevo(?:/|$)', r'-nuevo-', r'-nuevos-'],
        ...
        'color': {
            'category': 'color',
            'patterns': [r'negro', r'blanco', r'azul', r'rojo', r'verde', r'rosa', 
                        r'dorado', r'plata', r'gris', r'naranja', r'morado', r'amarillo'],
        ...
        'power': {
            'category': 'potencia',
            'patterns': [r'(?:\d+)-?w(?:/|$)', r'vatios', r'watts', r'potencia'],
            'suggested_name': 'Potencia',
            'adobe_filter_prefix': 'potencia:',
        },
        'processor': {
            'category': 'rendimiento',
            'patterns': [r'intel', r'amd', r'ryzen', r'core-i\d', r'snapdragon', r'mediatek', r'apple-m\d'],
    ...
    # Marcas conocidas por categoría (se usan para detección dinámica)
    KNOWN_BRANDS = {
        'tech': ['apple', 'samsung', 'xiaomi', 'huawei', 'oppo', 'realme', 'motorola', 
                 'google', 'honor', 'poco', 'nothing', 'oneplus', 'sony', 'lg', 'asus',
                 'lenovo', 'hp', 'dell', 'acer', 'msi', 'razer', 'corsair', 'logitech'],
        'appliances': ['bosch', 'siemens', 'balay', 'lg', 'samsung', 'whirlpool', 'electrolux',
                       'aeg', 'miele', 'candy', 'beko', 'haier', 'hisense', 'teka', 'zanussi'],
        'hvac': ['daikin', 'mitsubishi', 'fujitsu', 'lg', 'samsung', 'panasonic', 'hitachi',
                 'carrier', 'toshiba', 'gree', 'haier', 'hisense'],
        'tools': ['bosch', 'makita', 'dewalt', 'milwaukee', 'hikoki', 'metabo', 'black-decker',
    ...
    def _detect_brands(self) -> List[str]:
        ...
        for brand in all_brands:
            ...
            if count > 5:  # Al menos 5 URLs con esta marca
                found_brands.append(brand)
        
        return sorted(found_brands, key=lambda b: sum(1 for u in self.urls if f'/{b}/' in u.lower()), reverse=True)
    ...
    def detect_all(self) -> List[FacetMapping]:
        ...
        if found_brands:
            brand_pattern = '|'.join([f'/{b}(/|$)' for b in found_brands[:30]])  # Limitar a 30 marcas
    ...
    def _add_facet_if_found(self, facet_id: str, facet_name: str, pattern: str, 
        ...
        try:
            ...
            ] if 'Código de respuesta' in self.crawl.columns else self.crawl[
                self.crawl['Dirección'].str.contains(pattern, case=False, na=False, regex=True)
            ]
            ...
            urls_404 = self.crawl[
                ...
                (self.crawl['Dirección'].str.contains(pattern, case=False, na=False, regex=True))
            ] if 'Código de respuesta' in self.crawl.columns else pd.DataFrame()
            
            # Solo añadir si hay URLs
            ...
            # Si hay error en el regex, ignorar esta faceta
            pass
    ...
    def detect_unknown_patterns(self) -> List[Dict]:
        ...
        for url in self.urls:
            # Extraer path después del dominio y categoría
            path = url.lower()
        ...
        for segment, data in all_segments.items():
            if data['count'] > 10:  # Al menos 10 URLs
                is_known = any(re.search(p, segment, re.IGNORECASE) for p in known_patterns if p)
    ...
    def add_custom_facet(self, name: str, pattern: str, category: str = 'custom') -> FacetMapping:
        """Añade una faceta personalizada"""
        facet_id = f"custom_{name.lower().replace(' ', '_')}"
        
        urls_200 = self.crawl[
            (self.crawl.get('Código de respuesta', 200) == 200) & 
            (self.crawl['Dirección'].str.contains(pattern, case=False, na=False, regex=True))
        ] if 'Dirección' in self.crawl.columns else pd.DataFrame()
        
        urls_404 = self.crawl[
            (self.crawl.get('Código de respuesta', 200) == 404) & 
            (self.crawl['Dirección'].str.contains(pattern, case=False, na=False, regex=True))
        ] if 'Dirección' in self.crawl.columns else pd.DataFrame()
...
def render_data_period_config(data_sources: Dict[str, Any]) -> List[DataSourceConfig]:
    ...
    Args:
        data_sources: Dict con información de archivos cargados
    
    Returns:
        Lista de DataSourceConfig
    """
    ...
    st.info("""
    Indica el período que cubre cada archivo. Esto ayuda a:
    - Contextualizar los análisis
    - Evitar comparar datos de diferentes períodos
...
def render_facet_mapping_ui(detected_facets: List[FacetMapping], 
    ...
    Returns:
        Lista de facetas verificadas/modificadas
    """
    if not HAS_STREAMLIT:
        raise ImportError("Streamlit no está instalado")
        ...
        if unknown_patterns:
            ...
            for pattern in unknown_patterns:
                with st.expander(f"❓ `{pattern['segment']}` ({pattern['count']} URLs)"):
                    ...
                    # Opción de crear faceta desde este patrón
                    if st.button(f"Crear faceta para '{pattern['segment']}'", key=f"create_{pattern['segment']}"):
                        st.session_state[f'new_facet_pattern'] = pattern['segment']
    ...
    with tab3:
        ...
        with col2:
            new_category = st.selectbox(
                "Categoría",
                options=['marca', 'tamaño', 'memoria', 'conectividad', 'estado', 
