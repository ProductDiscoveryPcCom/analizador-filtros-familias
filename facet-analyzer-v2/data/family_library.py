"""
Gestor de Biblioteca de Familias de Productos - v2.2
...
from .loaders import DataLoader, FileType, LoadResult, FileTypeDetector
...
class FamilyMetadata:
    ...
    updated_at: str
    base_url: str
    category_path: str = ""  # Ej: "/smartphone-moviles"
    ...
    # Archivos incluidos (nuevo formato)
    ...
    def to_dict(self) -> Dict:
        ...
        data = asdict(self)
        # Convertir FamilyFile a dict
        if self.files:
            data['files'] = {k: asdict(v) if hasattr(v, '__dataclass_fields__') else v 
    ...
    def from_dict(cls, data: Dict) -> 'FamilyMetadata':
        ...
        # Convertir files de dict a FamilyFile
...
class FamilyLibrary:
    ...
    # Mapeo de tipos de archivo a nombres de almacenamiento
    FILE_STORAGE_NAMES = {
    ...
    def _generate_id(self, name: str) -> str:
        ...
        slug = name.lower()
        for char in 'áéíóúñ':
            slug = slug.replace(char, {'á': 'a', 'é': 'e', 'í': 'i', 'ó': 'o', 'ú': 'u', 'ñ': 'n'}[char])
        slug = ''.join(c if c.isalnum() else '-' for c in slug)
        slug = '-'.join(filter(None, slug.split('-')))  # Eliminar guiones múltiples
    ...
    def _extract_category_path(self, base_url: str) -> str:
        ...
        if not base_url:
            return ""
        # Ej: https://www.pccomponentes.com/smartphone-moviles -> /smartphone-moviles
    ...
    def create_family(self,
        ...
        Args:
            ...
            crawl_file: Archivo de crawl (obligatorio o recomendado)
            adobe_urls_file: Tráfico por URL
            adobe_filters_file: Demanda por filtros
            gsc_file: Datos de GSC (si no está en crawl)
            semrush_file: Keywords de SEMrush
            keyword_planner_file: Keywords de Google Ads
            crawl_historical_file: Crawl de URLs históricas
        
        Returns:
            FamilyMetadata creado
        """
        # Generar ID
        ...
        # Guardar en índice primero (para que add_file_to_family funcione)
    ...
    def load_family_data(self, family_id: str) -> Dict[str, pd.DataFrame]:
        ...
        # Cargar crawl maestro
        if metadata.has_crawl_master:
            # Preferir versión procesada (parquet)
            processed_path = family_path / 'crawl_master_processed.parquet'
            if processed_path.exists():
                data['crawl_adobe'] = pd.read_parquet(processed_path)
            else:
                csv_path = family_path / 'crawl_master.csv'
                if csv_path.exists():
                    result = loader.load_file(str(csv_path), FileType.CRAWL_MASTER)
                    if result.success:
                        data['crawl_adobe'] = result.dataframe
                        # Guardar procesado para próxima vez
                        result.dataframe.to_parquet(processed_path, index=False)
        
        # Cargar crawl con GSC (si existe y no hay crawl maestro)
        if metadata.has_crawl_gsc and 'crawl_adobe' not in data:
            csv_path = family_path / 'crawl_gsc.csv'
            if csv_path.exists():
                result = loader.load_file(str(csv_path), FileType.CRAWL_SF_GSC)
                if result.success:
                    data['crawl_gsc'] = result.dataframe
        
        # Cargar Adobe URLs
        if metadata.has_adobe_urls:
            csv_path = family_path / 'adobe_urls.csv'
            if csv_path.exists():
                result = loader.load_file(str(csv_path), FileType.ADOBE_URLS)
                if result.success:
                    data['adobe_urls'] = result.dataframe
        
        # Cargar Adobe Filters
        if metadata.has_adobe_filters:
            csv_path = family_path / 'adobe_filters.csv'
            if csv_path.exists():
                result = loader.load_file(str(csv_path), FileType.ADOBE_FILTERS)
                if result.success:
                    data['adobe_filters'] = result.dataframe
        
        # Cargar SEMrush
        if metadata.has_semrush:
            csv_path = family_path / 'semrush.csv'
            if csv_path.exists():
                result = loader.load_file(str(csv_path), FileType.SEMRUSH)
                if result.success:
                    data['semrush'] = result.dataframe
        
        # Cargar Keyword Planner
        if metadata.has_keyword_planner:
            csv_path = family_path / 'keyword_planner.csv'
            if csv_path.exists():
                result = loader.load_file(str(csv_path), FileType.KEYWORD_PLANNER)
                if result.success:
                    data['keyword_planner'] = result.dataframe
        
        # Cargar Crawl Histórico
        if metadata.has_crawl_historical:
            csv_path = family_path / 'crawl_historical.csv'
            if csv_path.exists():
                result = loader.load_file(str(csv_path), FileType.CRAWL_HISTORICAL)
                if result.success:
                    data['crawl_historical'] = result.dataframe
    ...
    def update_family(self, 
        ...
        Args:
            family_id: ID de la familia
            **file_kwargs: Archivos a actualizar (crawl_file, adobe_urls_file, etc.)
        
        Returns:
            FamilyMetadata actualizado
        """
        if family_id not in self.families:
            raise ValueError(f"Familia '{family_id}' no encontrada")
        
        # Mapeo de kwargs a FileType
        kwarg_to_type = {
            'crawl_file': FileType.CRAWL_MASTER,
            'adobe_urls_file': FileType.ADOBE_URLS,
            'adobe_filters_file': FileType.ADOBE_FILTERS,
        ...
        for file_type, file_info in metadata.files.items():
            if isinstance(file_info, FamilyFile):
                ...
                info = {
                    ...
                    'exists': (family_path / file_info).exists()
                }
