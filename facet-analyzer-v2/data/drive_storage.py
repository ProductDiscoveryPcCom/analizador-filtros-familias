...
class HybridLibraryStorage:
    ...
    def sync_from_drive(self) -> int:
        ...
        try:
            ...
            # Si falla la sincronización, no hacer nada
            pass
    ...
    def list_families(self, include_drive: bool = True) -> List[Dict]:
        ...
        for family_id, metadata in self.index.items():
            # Asegurar que metadata es un dict
            if isinstance(metadata, dict):
                ...
                # Si metadata no es dict, crear entrada básica
                families.append({
                    'id': family_id,
                    'source': 'local',
                    'name': family_id
        ...
        if include_drive and self.is_drive_enabled():
            try:
                ...
                # Si falla Drive, continuar con familias locales
                pass
