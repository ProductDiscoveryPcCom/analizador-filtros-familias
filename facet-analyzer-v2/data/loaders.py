...
class FileTypeDetector:
    ...
    def detect(cls, df: pd.DataFrame, filename: str = "") -> FileType:
        ...
        if 'keywordplanner' in filename_lower or 'keyword_planner' in filename_lower or 'kwr-' in filename_lower:
            return FileType.KEYWORD_PLANNER
        
        if 'topquery' in filename_lower or 'top_query' in filename_lower:
            # TopQuery de GSC - tratar como CRAWL_SF_GSC
            return FileType.CRAWL_SF_GSC
        
        if 'search_filter' in filename_lower or 'searchfilter' in filename_lower:
            return FileType.ADOBE_FILTERS
        
        if ('entry_page' in filename_lower or 'categorias' in filename_lower) and 'adobe' in filename_lower:
        ...
        # 7. Detectar Adobe por patrones de datos (última prioridad)
        ...
        # Verificar si la primera columna contiene URLs sin otras columnas específicas
        looks_like_url = 'http' in first_val or first_val.startswith('/') or 'www.' in first_val
        
        # Solo clasificar como ADOBE_URLS si NO tiene columnas típicas de otros formatos
...
class DataLoader:
    ...
    def load_file(self, filepath: str, file_type: FileType = None) -> LoadResult:
        ...
        skip_rows = 0
        filename_lower = filename.lower()
        is_adobe_file = any(x in filename_lower for x in ['adobe', 'sesiones', 'filtro', 'entry_page', 'search_filter', 'searchfilter', 'categorias'])
        is_kp_file = any(x in filename_lower for x in ['keywordplanner', 'keyword_planner', 'kwr-'])
        ...
        if is_adobe_file:
            ...
            skip_rows = 2  # Keyword Planner siempre tiene 2 filas de cabecera
            warnings.append("Detectadas 2 filas de cabecera Keyword Planner")
    ...
    def _process_adobe_urls(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        ...
        # Renombrar columnas conocidas de Adobe
        col_mapping = {}
        numeric_col_count = 0
        ...
        for i, col in enumerate(df.columns):
            ...
            # Primera columna con URLs
            if any(x in col_str for x in ['url', 'page', 'entry']):
                col_mapping[col] = 'url'
            # Columnas numéricas (totales usados como header) - asignar por posición
        ...
        if col_mapping:
            ...
            warnings.append(f"Columnas renombradas: {list(col_mapping.values())}")
        
        # Si aún no tiene 'url', verificar si la primera columna contiene URLs
        if 'url' not in df.columns:
            first_val = str(df.iloc[0, 0]).lower() if len(df) > 0 else ''
            if 'www.' in first_val or 'http' in first_val or '.com' in first_val:
        ...
        for col in numeric_cols:
            if col in df.columns:
                # Limpiar formato de números españoles
                df[col] = df[col].astype(str).str.replace('.', '', regex=False)  # Miles
                df[col] = df[col].astype(str).str.replace(',', '.', regex=False)  # Decimal
    ...
    def _process_adobe_filters(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        ...
        for i, col in enumerate(df.columns):
            ...
            # Primera columna con nombres de filtros
            if any(x in col_str for x in ['filter', 'search', 'facet']):
                col_mapping[col] = 'filter_name'
            # Segunda columna que es un número (total de visitas usado como header)
        ...
        # Si aún no tiene filter_name, verificar si la primera columna contiene filtros
        if 'filter_name' not in df.columns:
            first_val = str(df.iloc[0, 0]) if len(df) > 0 else ''
            if ':' in first_val:
                # La primera columna contiene filtros en formato faceta:valor
                old_name = df.columns[0]
                df = df.rename(columns={old_name: 'filter_name'})
                
                # Si la segunda columna es un número, es visits_seo
        ...
        if 'visits_seo' in df.columns:
            df['visits_seo'] = df['visits_seo'].astype(str).str.replace('.', '', regex=False)  # Miles español
            df['visits_seo'] = df['visits_seo'].astype(str).str.replace(',', '', regex=False).str.strip()
