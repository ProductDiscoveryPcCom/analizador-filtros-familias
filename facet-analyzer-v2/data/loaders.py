...
class FileTypeDetector:
    ...
    def detect(cls, df: pd.DataFrame, filename: str = "") -> FileType:
        ...
        if any(x in filename_lower for x in ['keywordplanner', 'keyword_planner', 'kwr-']):
            return FileType.KEYWORD_PLANNER
        
        if any(x in filename_lower for x in ['topquery', 'top_query']):
            return FileType.CRAWL_SF_GSC
        
        if any(x in filename_lower for x in ['search_filter', 'searchfilter']):
            return FileType.ADOBE_FILTERS
        
        if any(x in filename_lower for x in ['entry_page', 'categorias']) and 'adobe' in filename_lower:
        ...
        if len(df) > 0:
            ...
            looks_like_url = any(x in first_val for x in ['http', 'www.', '.com', '.es'])
...
class DataLoader:
    ...
    def load_file(self, filepath: str, file_type: FileType = None) -> LoadResult:
        ...
        skip_rows = 0
        filename_lower = filename.lower()
        is_adobe_file = any(x in filename_lower for x in [
            'adobe', 'sesiones', 'filtro', 'entry_page', 
            'search_filter', 'searchfilter', 'categorias'
        ])
